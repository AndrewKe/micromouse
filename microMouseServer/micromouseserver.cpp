...
void microMouseServer::shiftRight() {
    turnRight();
    moveForward();
    turnLeft();
    x++;
...
void microMouseServer::shiftUp() {
    moveForward();
    y++;
}

void microMouseServer::shiftLeft() {
    turnLeft();
    moveForward();
    turnRight();
    x--;
...
void microMouseServer::shiftDown(){
    turnRight();
    turnRight();
    moveForward();
    turnLeft();
...
bool microMouseServer::isWallBackward() {
    turnRight(); turnRight();
    bool ret = isWallForward();
    turnLeft(); turnLeft();
    return ret;
}

bool microMouseServer::valid(int x, int y) {
...
int microMouseServer::rating(int x, int y) {

    std::stringstream out;
    //out << "x" << x << "y" << y;
    printUI(out.str().c_str());
...
*/
    ...
    printUI(std::to_string(map[y][x]).c_str());
    map[y][x] = map[y][x]+1;
...
//    }
    ...
    int left = isWallLeft() ? 10000000 : rating(x-1, y);
    int right = isWallRight() ? 10000000 : rating(x+1, y);
    int bottom = isWallBackward() ? 10000000 : rating(x, y-1);
    int forward = isWallForward() ? 10000000 : rating(x, y+1);

    using namespace std;

    //std::stringstream out2;
    //out2 << " right: " << right << " bottom: " << bottom << " forward: " << forward << " left: " << left;
    //printUI(out2.str().c_str());

    if (!checking) {
        bool moved = false;
        if (forward <= bottom && forward <= left && forward <= right){
            shiftUp();
            printUI("UP");
            moved = true;
        }

        if (!moved && bottom <= forward && bottom <= left && bottom <= right){
            shiftDown();
            printUI("DOWN");
            moved = true;
        }

        if (!moved && left <= forward && left <= right && left <= bottom){
            shiftLeft();
            printUI("LEFT");
            moved = true;
        }

        if (!moved && right <= forward && right <= left && right <= bottom){
            shiftRight();
            printUI("RIGHT");
            moved = true;
        }
    ...
    if (checking) {
        if (back) {
          ...
          if (stage == 0 && !isWallForward() && !isWallRight()){
              shiftUp();
              stage++;
              return;
          }else if (stage == 1 && !isWallRight() && !isWallBackward()){
              shiftRight();
              stage++;
              return;
          }else if (stage == 2 && !isWallBackward()){
              foundFinish();
              stage++;
              return;
          }else {
